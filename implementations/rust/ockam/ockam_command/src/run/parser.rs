...
impl ConfigRunner {
    ...
    pub fn go(cli_state: &CliState, path: &Path) -> miette::Result<()> {
        ...
        cr.parse(cli_state, &config)?;
        cr.run()?;
        Ok(())
    }

    pub fn go_inline(cli_state: &CliState, config: &str) -> miette::Result<()> {
        let mut cr = Self::new();
        cr.parse(cli_state, config)?;
        cr.run()?;
        Ok(())
    ...
    fn parse(&mut self, cli_state: &CliState, config: &str) -> miette::Result<()> {
...
impl NodeConfig {
    fn parse(
        ...
        let mut insert_command =
            |subject: &str, name: &str, depends_on, args: &[&str], background: bool| {
...
mod tests {
    ...
    fn test_parse_config_with_depends_on() {
        ...
        sut.parse(&cli_state, config).unwrap();
    ...
    fn detect_circular_dependency() {
        ...
        for (config, expected) in cases {
            let mut sut = ConfigRunner::new();
            let cli_state = CliState::test().unwrap();
            let result = sut.parse(&cli_state, config);
            match expected {
